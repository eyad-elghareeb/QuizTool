...
(function () {
  ...
  function initStyles() {
    ...
    style.textContent = [
      ...
      '@media (max-width:560px){.start-screen{padding:14px}.start-card{padding:20px}.api-row,.start-actions,.answer-foot{align-items:stretch;flex-direction:column}.start-actions .btn,.answer-foot .btn,.manual-bar .btn{width:100%}}'
    ].join('\n');
  ...
  function initContent() {
    ...
    if (!_readKey()) {
      setTimeout(function () {
        showToast('⚙ Set your Gemini API key in Settings for AI grading');
      }, 500);
    }
  }
  ...
  function renderFeedback(evaluation) {
    ...
    if (photo && photo.data) {
      ...
      if (evaluation.transcription) {
        html += '<div class="transcription-box"><div class="transcription-label">AI Transcription</div><div class="transcription-text">' + escapeHtml(evaluation.transcription) + '</div></div>';
      }
